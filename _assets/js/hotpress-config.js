$(function () {
	...
	var mainNav = $("#nav > ul > li > a");
	// cycle through each link on the main nav. If the href equals the first part of the pathname add the class selected
	
	var foundMe = false;
	
	for (var i=0; i<mainNav.length; i++){
		var href = $(mainNav[i]).attr("href").split( '/' );
	    var hrefPathArray = "/" + href[1];		
		if(hrefPathArray == firstPathArray){
			foundMe = true;
			$(mainNav[i]).parent().addClass("selected");
		}
	}		
	if(!foundMe){
		var selectId = window.selectedStatePageID;
		if(selectId != undefined){ 
		
			for (var i=0; i<mainNav.length; i++){
			var thishref = $(mainNav[i]).attr("href").split( '/' );
			var itemNeeded = "/" + thishref[1];		
			if(itemNeeded == selectId){
				foundMe = true;
				$(itemNeeded).addClass("selected");
			}
		}
